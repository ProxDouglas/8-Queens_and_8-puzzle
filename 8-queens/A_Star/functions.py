...
def attacked_queens_pairs(seqs):
    ...
    for i in range(1, 9):
        ...
        for j in range(i + 1, 9):
            tentativas = tentativas + 1  # Look at the two diagonals in the right half
            if t1 != 1:
                t1 -= 1
                if a[t1][j] == 1:
    ...
    return int(n/2), tentativas  # Returns n/2, because A attacking B also means B attacking A, so returns half of n
