...
while frontier_priority_queue: # If the frontier is not empty, the loop will continue. If the solution is found successfully, the loop will output the solution. If the frontier is empty, the loop will fail
    estados = frontier_priority_queue.pop(0)  # Since frontier s are sorted every time, the first sequence is extended
    nodes = nodes + 1
    if estados['pairs'] == 0 and estados['unplaced_queens'] == 0: # Do the goal test before extending the node: if the sequence h(n)=g(n)=0, then the sequence is the solution sequence
        solution = estados['seqs']
        flag = 1  # success
        break
